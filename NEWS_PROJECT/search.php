            ...
            <div class="post-container">
              ...
              <?php
                if(isset($_GET['search']) && $search!="") {
                  ...
                  include "config.php";
                  $limit = 4;
                  $sql2 = "SELECT * FROM post WHERE title LIKE '%$search%' or description LIKE '%$search%'";
                  ...
                  $sql3 = "SELECT p.post_id,p.title,p.description,p.category,p.post_date,p.post_img,p.author,c.category_name,u.first_name,u.last_name FROM post AS p INNER JOIN category AS c ON p.category=c.category_id INNER JOIN user_t AS u ON p.author=u.user_id WHERE p.title LIKE '%$search%' or p.description LIKE '%$search%' ORDER BY p.numbering DESC LIMIT $offset,$limit";
                  $result3 = mysqli_query($conn,$sql3) or die("<h1>Query Failed1</h1>");
                  mysqli_close($conn);
              ...
              <?php
                if(isset($_GET['search']) && $search!="") {
                  ...
                  include "config.php";
                  $sql4 = "SELECT * FROM post WHERE title LIKE '%$search%' or description LIKE '%$search%'";
                  $result4 = mysqli_query($conn,$sql4) or die("<h1>Query Failed2</h1>");;
                  mysqli_close($conn);
                  ...
                  if($total_no_of_record > 0) {
                    ...
                    if($total_page > 1) {
                      ...
                      if($page>1) {
                        $p=$page-1;
                        echo "<a href='search.php?search=$search&page=$p'>Prev</a>";
                      }
                      else {
                      ...
                      for($j=1;$j<=$total_page;$j++) {
                        ...
                        if($page==$j) {
                          echo "<a class='active' href='search.php?search=$search&page=$j'>$j</a>";
                        }
                        else {
                          echo "<a href='search.php?search=$search&page=$j'>$j</a>";
                        }
                      }

                      if($total_page>$page) {
                        $p=$page+1;
                        echo "<a href='search.php?search=$search&page=$p'>Next</a>";
                      }
                      else {
