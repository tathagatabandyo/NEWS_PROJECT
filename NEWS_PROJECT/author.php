            ...
            <div class="post-container">
              ...
              <?php
                ...
                if(isset($_GET['aid'])) {
                  ...
                  if($check) {
                    $sq = "SELECT first_name,last_name FROM user_t WHERE user_id='$aid'";
                    $res = mysqli_query($conn,$sq) or die("<h1>Query Failed</h1>");
                    $ar = mysqli_fetch_assoc($res);
                    echo "<h2 class='page-heading'>{$ar['first_name']} {$ar['last_name']}</h2>";
                  }
                }
              ?>
              <!-- Category Name end -->
              ...
              <?php
                if(isset($_GET['aid'])) {
                  ...
                  if($check) {
                    // include "config.php";
                    $limit = 2;
                    $sql2 = "SELECT * FROM post WHERE author='$aid'";
                    ...
                    if(isset($_GET['page'])) {
                      ...
                      if($page=="0" || $page=="" || $t_p<$page) {
                        mysqli_close($conn);
                        $said = sha1($aid);
                        header("Location: $hostname/author.php?aid=$said");
                      }
                    }
                    else {
                      $page=1;
                    ...
                    $sql3 = "SELECT p.post_id,p.title,p.description,p.category,p.post_date,p.post_img,p.author,c.category_name,u.first_name,u.last_name FROM post AS p INNER JOIN category AS c ON p.category=c.category_id INNER JOIN user_t AS u ON p.author=u.user_id WHERE p.author='$aid' ORDER BY p.numbering DESC LIMIT $offset,$limit";
              ...
              <?php
                    ...
                    echo "
                    <div class='post-content single-post'>
                      <h3>No Author Record Found.<span>(Don't change Author id)</span></h3>
                    </div>";
                  }
                }
                else {
              ...
              <?php
                if(isset($_GET['aid'])) {
                  if($check) {
                    include "config.php";
                    $sql4 = "SELECT * FROM post WHERE author='$aid'";
                    ...
                    if($total_no_of_record > 0) {
                      ...
                      if($total_page > 1) {
                        ...
                        if($page>1) {
                          $p=$page-1;
                          echo "<a href='author.php?aid=$said&page=$p'>Prev</a>";
                        }
                        else {
                        ...
                        for($j=1;$j<=$total_page;$j++) {
                          ...
                          if($page==$j) {
                            echo "<a class='active' href='author.php?aid=$said&page=$j'>$j</a>";
                          }
                          else {
                            echo "<a href='author.php?aid=$said&page=$j'>$j</a>";
                          }
                        }

                        if($total_page>$page) {
                          $p=$page+1;
                          echo "<a href='author.php?aid=$said&page=$p'>Next</a>";
                        }
                        else {
